#!/bin/bash
# Batch 13Jun fix — solo i 3 contratti rimasti nonDerivable

set -euo pipefail

BASE="/home/labeconomia/mdiianni/verimap_projects/swipl_z3_clpq_interpreter"
SCRIPT="$BASE/script"
LOG="$BASE/test/batch_13Jun_fix.log"
> "$LOG"
...
run_test() {
    ...
    # Rimuove artefatti precedenti
    rm -f "$DIR"/*.pl "$DIR"/*.json "$DIR"/*.zmiout "$DIR"/*.txt "$DIR"/*.t.pl* "$DIR"/*.t_constr.pl
...
log ""
log "=== RIEPILOGO ==="
for tag in "few_calls_buggy2" "seq_calls_buggy2" "state_machine_1_buggy2"; do
    d="$BASE/test/13Jun_$tag"
    zmiout=$(ls "$d"/*.zmiout 2>/dev/null | head -1 | xargs -I{} basename {} || echo "ERRORE")
