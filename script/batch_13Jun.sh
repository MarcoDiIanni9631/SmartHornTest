#!/bin/bash
# Batch 13Jun — contratti buggy2: aggiunto assert violabile su quelli nonDerivable

set -euo pipefail

BASE="/home/labeconomia/mdiianni/verimap_projects/swipl_z3_clpq_interpreter"
SCRIPT="$BASE/script"
LOG="$BASE/test/batch_13Jun.log"
> "$LOG"
...
run_test() {
    ...
    local sol_name="$2"
    local hint="$3"
    local DIR="$BASE/test/13Jun_$tag"
    ...
    RESULT=$(ls "$DIR"/*.zmiout 2>/dev/null | head -1 | xargs -I{} basename {} || echo "ERRORE")
    log "  RISULTATO: $RESULT"
    log "████ DONE: $tag ████"
    echo "" >> "$LOG"
}

log "=== BATCH 13Jun AVVIATO (buggy2 — assert violabile) ==="
log ""

run_test "simple_if_buggy2"         "simple_if.sol"      "simple_if"
run_test "for1correct_buggy2"       "for_1_correct.sol"  "f"
run_test "loop_if_buggy2"           "loop_if.sol"        "loop_if"
run_test "few_calls_buggy2"         "few_calls.sol"      "f"
run_test "many_fun_buggy2"          "many_fun.sol"       "f5"
run_test "seq_calls_buggy2"         "seq_calls.sol"      "call_1"
run_test "simple_if_2_buggy2"       "simple_if_2.sol"    "simple_if_2"
run_test "state_machine_1_buggy2"   "state_machine_1.sol" "i"

log ""
log "=== BATCH COMPLETATO ==="
log ""
log "=== RIEPILOGO ==="
for d in "$BASE/test/13Jun_"*; do
    tag=$(basename "$d" | sed 's/13Jun_//')
    zmiout=$(ls "$d"/*.zmiout 2>/dev/null | head -1 | xargs -I{} basename {} || echo "ERRORE/NESSUNO")
    if echo "$zmiout" | grep -q "derivable"; then
        log "  ✓ $tag: DERIVABLE"
    else
        log "  ✗ $tag: $zmiout"
    fi
